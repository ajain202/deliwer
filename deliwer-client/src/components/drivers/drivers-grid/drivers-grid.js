...
const DriversGrid = ({ searchParam, favorites, sortByValue }) => {
  ...
  const setFavoriteDriver = (e) => {
    setDrivers(
      drivers.map((d) =>
        d.name === e.currentTarget.getAttribute("driver-name")
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {drivers
        ?.filter(
          (driver) => driver.name.toLowerCase().indexOf(searchParam) > -1
        )
        .filter((driver) => (favorites ? driver.favorite && driver : driver))
        .sort((a, b) => {
          if (sortByValue === "Orders delivered for you")
            return b.yourOrders - a.yourOrders;
          else if (sortByValue === "Total orders delivered")
