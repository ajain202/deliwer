          ...
          <div>
            <p className="text-xl font-semibold leading-4 ">
              <DropdownInput
                ...
                setValue={(value) => setCurrentMonth(value)}
              />
            </p>
            <p className="text-md leading-3 pt-4 pb-1">
              Monthly Expenditure:{' '}
              <span className="text-base font-medium leading-none">{month.filter((m) => m.month === currentMonth)[0].expenses + '$'}</span>
            </p>
            <p className="text-md leading-none pt-5">{month.filter((m) => m.month === currentMonth)[0].delta + '% less than last month'}</p>
          </div>
        </div>
      </div>
    </div>
